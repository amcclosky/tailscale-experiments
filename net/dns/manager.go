...
type dnsTCPSession struct {
	...
	queries   chan []byte // DNS requests from reading

	ctx      context.Context
	closeCtx context.CancelFunc
}

func (s *dnsTCPSession) handle() {
	defer s.conn.Close()
...
func (s *dnsTCPSession) handleQuery(q []byte) {
	resp, err := s.m.Query(s.ctx, q, s.from)
	if err != nil {
...
func (s *dnsTCPSession) readRoutine() {
	defer close(s.queries)
...
func (m *Manager) HandleTCPConn(conn net.Conn, from netaddr.IPPort) {
	s := dnsTCPSession{
		m:         m,
		conn:      conn,
		from:      from,
